...
public class Jinx {
     ...
     *
     * <br>
     * Flickr recommends that all calls to their API contain a User-Agent in the
     * header that identifies the application using the API. This method is
     * deprecated in favor of the method that includes a value
     * for the user agent. If you use this method, Jinx will set the user agent to
     * "Jinx/version".
     *
     * @param apiKey      the API key to use.
     * @param apiSecret   the API secret to use.
     * @param permissions the requested Flickr permissions.
     * @deprecated You should use the constructor that includes the userAgent parameter.
     */
    @Deprecated
    public Jinx(String apiKey, String apiSecret, JinxConstants.OAuthPermissions permissions) {
        this(apiKey, apiSecret, null, permissions, null);
    }
     ...
     * @param apiSecret   the API secret to use.
     * @param permissions the requested Flickr permissions.
     * @param userAgent   the user agent to set in the header.
     */
    public Jinx(String apiKey, String apiSecret, JinxConstants.OAuthPermissions permissions, String userAgent) {
        this(apiKey, apiSecret, null, permissions, userAgent);
    }
    ...
    public Jinx(String apiKey, String apiSecret, OAuthAccessToken oAuthAccessToken, JinxConstants.OAuthPermissions permissions) {
        this(apiKey, apiSecret, oAuthAccessToken, permissions, null);
    }
     ...
     * <br>
     * Flickr recommends that all calls to their API contain a User-Agent in the
     * header that identifies the application using the API.
     * If you do not provide a user agent, Jinx will set the user agent to
     * "Jinx/version".
     *
     * @param apiKey           the API key to use.
     * @param apiSecret        the API secret to use.
     ...
     * @param userAgent        the user agent to set in the header.
     */
    public Jinx(String apiKey, String apiSecret, OAuthAccessToken oAuthAccessToken, JinxConstants.OAuthPermissions permissions, String userAgent) {
        this.apiKey = apiKey;
        this.apiSecret = apiSecret;
        this.oAuthAccessToken = oAuthAccessToken;
        if (userAgent == null || userAgent.isBlank()) {
            this.userAgent = String.format("%s/%s",
                    Optional.ofNullable(Jinx.class.getPackage().getImplementationTitle()).orElse("Jinx"),
                    Optional.ofNullable(Jinx.class.getPackage().getImplementationVersion()).orElse("0.0.0"));
        } else {
    ...
    protected <T> T callFlickr(Map<String, String> params, Method method, Class<T> tClass, boolean sign) throws JinxException {
        ...
        try {
            if (method == Method.GET) {
                ...
                request.addHeader(USER_AGENT_HEADER_KEY, userAgent);
                if (verboseLogging) {
                    JinxLogger.getLogger().log(String.format("Added header parameter %s=%s",
                            USER_AGENT_HEADER_KEY, userAgent));
                }
